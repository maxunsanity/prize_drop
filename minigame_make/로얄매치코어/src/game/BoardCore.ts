...
export type BoardEvent =
  ...
  | { type: 'BONUS_TIME_END' }
  | { type: 'ITEM_USE'; item: 'HAMMER' | 'SHUFFLE'; row: number; col: number; targets: Block[] }
...
export class BoardCore {
  ...
  /* ── 아이템 사용 ── */
  useItem(item: 'HAMMER' | 'SHUFFLE', row: number, col: number): void {
    if (this._locked) return;
    ...
    if (item === 'HAMMER') {
      ...
      setTimeout(() => this._dropAndSpawn(), 450);
    }
      ...
      // 그리드에서 일단 전부 제거
      for (const b of blocks) this.grid[b.row][b.col] = null;
