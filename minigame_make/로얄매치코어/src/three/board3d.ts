...
export class Board3D {
  ...
  private _onBoardEvent(e: BoardEvent): void {
    switch (e.type) {
      ...
      case 'SUCCESS':
        ...
        break;
      case 'ITEM_USE':
        this._animItemUse(e.item, e.targets, e.row, e.col);
        break;
      default:
        break;
    }
  ...
  private _animItemUse(
    ...
    if (item === 'HAMMER') {
      ...
      hammer.position.set(wx + 1.2, wy + 1.5, 2.0);
      ...
      const swing = (): void => {
        if (this.disposed) { this.scene.remove(dimMesh, hammer); return; }
        const t = Math.min(1, (this.clock.getElapsedTime() - startT) / dur);
        ...
        if (t < 1) {
          ...
          const ringAnim = (): void => {
            if (this.disposed) { this.scene.remove(ring); return; }
            const rt = Math.min(1, (this.clock.getElapsedTime() - rStart) / 0.25);
            ring.scale.setScalar(1 + rt * 3.5);
          ...
          // 블록 파괴 — squash-stretch 소멸 애니메이션
          let colA = 0xffd700;
          if (targets.length > 0) {
            const cfg = this.configMap.get(targets[0].colorType);
            if (cfg) colA = cfg.particleA;
          }
          burstAtBlock(this.particlePool, wx, wy, colA, 0xffffff, 24);
          this._animExplode(targets, 1);  // squash-stretch → _removeMesh → _spawnHoleEffect

          // 망치 튕겨 사라지기 (180ms)
          const bounceStart = this.clock.getElapsedTime();
          const bEnd = new THREE.Vector3(wx + 0.4, wy + 0.6, 1.2);
          const bounce = (): void => {
            if (this.disposed) { this.scene.remove(dimMesh, hammer); return; }
            const bt = Math.min(1, (this.clock.getElapsedTime() - bounceStart) / 0.18);
            hammer.position.lerpVectors(targetPos, bEnd, bt);
            hammer.rotation.z = -Math.PI / 6 * bt;
            hammer.traverse(child => {
              if (child instanceof THREE.Mesh && child.material) {
                (child.material as THREE.MeshBasicMaterial).transparent = true;
                (child.material as THREE.MeshBasicMaterial).opacity = 1 - bt;
              }
            });
            if (bt < 1) {
              requestAnimationFrame(bounce);
            } else {
              this.scene.remove(dimMesh, hammer);
    ...
    /* ─── 🔀 셔플: 블록 전부 스케일 다운 → 새 위치로 이동 → 스케일 업 ─── */
    else if (item === 'SHUFFLE') {
      // targets = 셔플된 블록 목록 (이미 새 row/col 적용됨)
      // mesh는 아직 원래 위치에 있음
      const meshMoves: { mesh: THREE.Mesh; block: Block; toX: number; toY: number }[] = [];
      for (const b of targets) {
        const mesh = this.blockMeshes.get(b.id);
        if (!mesh) continue;
        const [toX, toY] = gridToWorld(b.row, b.col);
        meshMoves.push({ mesh, block: b, toX, toY });
      }
      if (meshMoves.length === 0) return;

      audio.tap();
      const scaleDownDur = 0.20;  // 200ms 축소
      const scaleUpDur   = 0.25;  // 250ms 확대
      const total        = scaleDownDur + scaleUpDur;
      const animStart    = this.clock.getElapsedTime();
      let   teleported   = false;

      // 화면 중앙 파티클 폭발 (셔플 시작 느낌)
      burstAtBlock(this.particlePool, 0, 0, 0xffffff, 0x88aaff, 20);

      const animShuffle = (): void => {
        if (this.disposed) return;
        const elapsed = this.clock.getElapsedTime() - animStart;

        if (elapsed < scaleDownDur) {
          // 1단계: 스케일 축소
          const s = 1 - easeInOut(elapsed / scaleDownDur);
          meshMoves.forEach(({ mesh }) => mesh.scale.set(s, s, 1));
        } else {
          if (!teleported) {
            // 중간 시점: 메쉬를 새 위치로 순간이동 (스케일=0 상태)
            teleported = true;
            meshMoves.forEach(({ mesh, block, toX, toY }) => {
              mesh.scale.set(0, 0, 1);
              mesh.position.set(toX, toY, 0.05);
              mesh.userData['row']   = block.row;
              mesh.userData['col']   = block.col;
              mesh.userData['baseY'] = toY;
            });
          ...
          // 2단계: 새 위치에서 스케일 확대
          const s = easeInOut(Math.min(1, (elapsed - scaleDownDur) / scaleUpDur));
          meshMoves.forEach(({ mesh }) => mesh.scale.set(s, s, 1));
        }

        if (elapsed < total) {
          requestAnimationFrame(animShuffle);
        } else {
          meshMoves.forEach(({ mesh }) => mesh.scale.set(1, 1, 1));
        }
      };
      requestAnimationFrame(animShuffle);
    }
